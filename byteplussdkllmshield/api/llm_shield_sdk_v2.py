...
class SessionTimeout(requests.Session):
    ...
    def request(self, method, url, **kwargs):
        ...
        if "timeout" not in kwargs and self.default_timeout is not None:
            # 流式请求避免 read timeout 打断连接：只设置 connect timeout
            if kwargs.get("stream") and isinstance(self.default_timeout, (int, float)):
                kwargs["timeout"] = (self.default_timeout, None)
            else:

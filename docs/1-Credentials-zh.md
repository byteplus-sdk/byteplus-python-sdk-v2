...
为保障资源访问安全，Byteplus Python SDK 支持显式凭证和 `CredentialProvider` 自动解析两种方式。
...
| `CLIConfigCredentialProvider` | 读取 `~/.byteplus/config.json` | 取决于 mode | 复用 CLI 配置 |
...
STS AssumeRole（Security Token Service）是Byteplus提供的临时访问凭证机制。开发者通过服务端调用 STS 接口获取临时凭证（临时 AK、SK 和 Token），有效期可配置，适用于安全要求较高的场景。
...
if __name__ == '__main__':
    ...
    configuration.credential_provider = StsCredentialProvider(
        ...
        host="sts.byteplusapi.com",  # 非必填，请求域名，默认sts.byteplusapi.com
        region="ap-southeast-1",  # 非必填，请求服务器区域地址，默认ap-southeast-1
        timeout=30,  # 非必填，请求超时时间，默认30秒
        expired_buffer_seconds=60, #非必填，session有效期前多久过期，剩余时间小于这个设置就要请求新的token了，默认60秒
        policy='{"Statement":[{"Effect":"Allow","Action":["vpc:CreateVpc"],"Resource":["*"],"Condition":{"StringEquals":{"byteplus:RequestedRegion":["ap-southeast-1"]}}}]}' # 非必填，授权策略，默认为空
    )
...
```python
# Example Code generated by Byteplus Technology.
from __future__ import print_function
import byteplussdkcore
import byteplussdkvpc
from byteplussdkcore.rest import ApiException
...
STS AssumeRoleWithSaml（Security Token Service）是Byteplus提供的临时访问凭证机制。开发者通过saml_token在服务端调用 STS 接口获取临时凭证（临时 AK、SK 和 Token），有效期可配置，适用于安全要求较高的场景。
...
os.environ["BYTEPLUS_ACCESS_KEY"] = "YourAK"
os.environ["BYTEPLUS_SECRET_KEY"] = "YourSK"
...
- 配置文件优先级：构造参数 `config_path` > `BYTEPLUS_CLI_CONFIG_FILE` > `~/.byteplus/config.json`
- Profile 优先级：构造参数 `profile_name` > `BYTEPLUS_PROFILE` > 配置里的 `current` > `default`
...
- `role_name` 优先级：构造参数 > `BYTEPLUS_ECS_METADATA` > 从 IMDS 自动探测
...
configuration = byteplussdkcore.Configuration()
configuration.region = "ap-southeast-1"
# 省略 role_name 时，将读取 BYTEPLUS_ECS_METADATA 或从 IMDS 自动探测角色名。
